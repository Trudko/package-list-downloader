    ...
    request(requestOptions, function (error, response, body) {
      if (!error && response.statusCode == 200) {
        ...
        wasListLoaded = true;
          ...
          new StatusView({type: 'alert', message: 'Gist with id ' + gistID + ' was not found!'})
          wasListLoaded = false;
      }
    });
  ...
  installPackage: function(packageName) {
      ...
      exit = (function(_this) {
        return function(exitCode) {
          if (exitCode === 0) {
            new StatusView({type: 'success', message: "" + packageName + " installed!"})
          } else {
            new StatusView({type: 'alert', message: "" + packageName + "An error occurred installing!"})
          }
        };
      })(this);
